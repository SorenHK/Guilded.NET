...
/// <summary>
/// Represents the base for any kinds of <see cref="BaseGuildedService">Guilded clients</see>.
/// </summary>
public abstract class BaseGuildedService : IDisposable
{
    ...
    protected BaseGuildedService(Uri apiUrl)
    {
        ...
        Rest = new RestClient(apiUrl ?? throw new ArgumentNullException(nameof(apiUrl)))
            ...
            .AddDefaultHeader("User-Agent", $"Guilded-NET GNET-{typeof(BaseGuildedService).Assembly.GetName().Version} CLR-{Environment.Version}")
    ...
    /// </remarks>
    /// <param name="request">The request to send to execute</param>
    /// <param name="channel"><see cref="T:Guilded.Servers.ServerChannel">The channel</see> where the <paramref name="request" /> is being executed</param>
    ...
    #region Methods Util
    /// <summary>
    /// Disposes the <see cref="Rest">REST client</see> of the <see cref="BaseGuildedService">service</see>.
    /// </summary>
